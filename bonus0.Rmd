...
keyPressDataWithLaneDeviation = read.csv("./data/keyPressDataWithLaneDeviation.csv")
...
buildRunOneTrial <- function(singleTaskKeyPressTimes, singleTaskKeyPressData = NULL) {
  runOneTrial <- function(strategy,nrSteeringUpdates,normalPhoneStructure,phoneStringLength,phoneNumber)   #### strategy stores where participant interleaves
  {
  	...
  	if(!is.null(singleTaskKeyPressData)){
  	  dialTimes = 1:phoneStringLength
  	  numberstring_split <- as.integer(strsplit(phoneNumber, "")[[1]])
      for (pos in dialTimes) {
  	    tmp_line <- singleTaskKeyPressData[pos,]
  	    tmp_mean <- tmp_line$meanDiffs
  	    tmp_std <- tmp_line$stdDiffs
        dialTimes[pos] <- abs(round(rnorm(n=1, tmp_mean, tmp_std)))
      }
...
num_sim = 1
generateChunkStrategiesModelbased <- buildGenerateChunkStrategiesModelbased(5)
runOneTrialStandrd <- buildRunOneTrial(rep(400, nchar(strPhoneNr)))
runOneTrialSlowest <- buildRunOneTrial(rep(round(typing_speed_slowest$val), nchar(strPhoneNr)))
runOneTrialFastest <- buildRunOneTrial(rep(round(typing_speed_fastest$val), nchar(strPhoneNr)))
runOneTrialAverage <- buildRunOneTrial(rep(round(typing_speed_average$val), nchar(strPhoneNr)))
runOneTrialVariableKeypress <- buildRunOneTrial(typing_speed_average_per_digit$meanDiffs, typing_speed_average_per_digit)
...
sim_data_var_kp <- runAllComplexStrategies(num_sim, strPhoneNr, generateChunkStrategiesSystematic, runOneTrialVariableKeypress)
sim_data_system <- runAllComplexStrategies(num_sim, strPhoneNr, generateChunkStrategiesSystematic, runOneTrialStandrd)
sim_data_Fastest <- runAllComplexStrategies(num_sim, strPhoneNr, generateChunkStrategiesSystematic, runOneTrialFastest)
sim_data_Slowest <- runAllComplexStrategies(num_sim, strPhoneNr, generateChunkStrategiesSystematic, runOneTrialSlowest)
sim_data_Average <- runAllComplexStrategies(num_sim, strPhoneNr, generateChunkStrategiesSystematic, runOneTrialAverage)
sim_data_specifics <- runAllComplexStrategies(num_sim, strPhoneNr, generateChunkStrategiesSpecifics, runOneTrialStandrd)
